...
#?(:clj (defn conformance-fixture
          "Fixture for running the conformance tests."
          [f]
          (binding [conf/*current-test-suite-dir*
                    (some-> (conf/clone-test-suite
                             :branch "LRS-2.0")
                            conf/install-test-suite!)]
            (try
              (f)
              (finally
                (conf/delete-test-suite! conf/*current-test-suite-dir*))))))
