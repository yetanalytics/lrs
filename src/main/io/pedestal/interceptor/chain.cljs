...
(ns io.pedestal.interceptor.chain
  ...
  handling. No support for async execution, forces sync."
...
(defn- channel? [c]
  (satisfies? cljs.core.async.impl.protocols/Channel c))
...
(defn- throwable->ex-info [t execution-id interceptor stage]
  (let [iname (name interceptor)
        throwable-str (pr-str (type t))]
    (ex-info (str throwable-str " in Interceptor " iname " - " (.-message t))
  ...
  (let [execution-id (::execution-id context)]
    (if-let [f (get interceptor stage)]
      (try (js-debug
           ...
           (f context)
           (catch js/Error t
             (js-debug :throw t :execution-id execution-id)
  ...
  (let [execution-id (::execution-id context)]
    (if-let [error-fn (get interceptor :error)]
      (let [ex (::error context)
            stage :error]
        (js-debug :interceptor (name interceptor)
         :stage :error
         :execution-id execution-id)
      ...
      (do (js-trace :interceptor (name interceptor)
                     :skipped? true
                     :stage :error
                     :execution-id execution-id)
          context))))
...
(defn- process-all-with-binding
  ...
  :io.pedestal.interceptor.chain/queue of context, saves them on the ::stack of context.
  Returns updated context.
  By default, `interceptor-key` is :enter"
  ([context]
   (process-all-with-binding context :enter))
  ([context interceptor-key]
   (js-debug :in 'process-all :handling interceptor-key :execution-id (::execution-id context))
   (loop [context context]
     (let [queue (::queue context)
           stack (::stack context)
           execution-id (::execution-id context)]
       (js-trace :context context)
       (if (empty? queue)
         context
...
(defn- process-any-errors-with-binding
  ...
  [context]
  (js-debug :in 'process-any-errors :execution-id (::execution-id context))
  (loop [context context]
...
(defn- leave-all-with-binding
  ...
  [context]
  (js-debug :in 'leave-all :execution-id (::execution-id context))
  (loop [context context]
    (let [stack (::stack context)
          execution-id (::execution-id context)]
      (js-trace :context context)
...
(defn enqueue
  ...
  (js-trace :enqueue (map name interceptors) :context context)
  (update-in context [::queue]
             (fnil into cljs.core.PersistentQueue.EMPTY)
             interceptors))
...
(defn terminate
  ...
  [context]
  (js-trace :in 'terminate :context context)
  (dissoc context ::queue))
...
(def execution-id (atom 0))

(defn- begin [context]
  (if (contains? context ::execution-id)
    context
    (let [execution-id (swap! execution-id inc)]
      (js-debug :in 'begin :execution-id execution-id)
      (js-trace :context context)
...
(defn- end [context]
  (if (contains? context ::execution-id)
    (do
      (js-debug :in 'end :execution-id (::execution-id context) :context-keys (keys context))
      (js-trace :context context)
