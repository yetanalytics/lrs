...
(defn dissoc-in [m ks]
  (update-in m (into [] (butlast ks)) dissoc (last ks)))
...
(defn decode-query-part
  ...
  #_(URLDecoder/decode string "UTF-8"))
...
(defn encode-query-part
  ...
  #_(URLEncoder/encode string "UTF-8"))
                ...
                (catch js/Error iae
                  (interceptor.chain/terminate
                    (assoc ctx :response {:status 400
                                          :body (str "Bad Request - " (.-message iae))})))))}))
...
(defn try-routing-for [spec router-type query-string verb]
  (let [router  (delegate-router spec router-type)
