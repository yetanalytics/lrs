...
(ns io.pedestal.http
  ...
  #_(:import (java.io OutputStreamWriter
                    OutputStream)))
...
#_(defn transit-body-interceptor
  ...
  (let [{type ::type
        ...
        server-fn (if (fn? type)
                    ...
                    (throw (ex-info "Only functions are allowed for service types in js."
                                    {:type ::invalid-type}))
                    #_(let [server-ns (symbol (str "io.pedestal.http." (name type)))]
                      (require server-ns)
...
(defn create-server
  ([service-map]
   (create-server service-map (constantly nil) #_log/maybe-init-java-util-log))
  ([service-map init-fn]
   (init-fn)
   (-> service-map
...
#_(defn servlet-destroy [service]
  (dissoc service ::servlet))

#_(defn servlet-service [service servlet-req servlet-resp]
